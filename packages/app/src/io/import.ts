...
import { isNotNullish, isString } from '@figureland/kit/tools/guards'
import { settle } from '@figureland/kit/tools/async'

export type ValidMimeType = (typeof NN_IMPORT_FORMATS)[number]
...
export const importFile = (file: File): Promise<ParsedEntity | null> =>
  new Promise(async (resolve) => {
    const fileType = file.type as ValidMimeType
    if (!NN_IMPORT_FORMATS.includes(fileType)) {
      resolve(null)
    }
