...
import { keys, isObject } from '@nodenogg.in/utils'
...
const vulnerableTags: string[] = ['script', 'style']
...
function filter<T>(a: T[], cb: (value: T) => boolean): T[] {
  return a.filter(cb)
}
...
export const sanitizeHtml = (
  ...
  class Frame {
    ...
    text: string
    mediaChildren: any[]
    constructor(tag: string, attribs: any) {
      this.tag = tag
  ...
  let allowedAttributesMap: any
  let allowedAttributesGlobMap: any
  if (options.allowedAttributes) {
    allowedAttributesMap = {}
    allowedAttributesGlobMap = {}
    each(options.allowedAttributes, (attributes: any, tag: string) => {
      allowedAttributesMap[tag] = []
      const globRegex: any[] = []
      attributes.forEach((obj: any) => {
