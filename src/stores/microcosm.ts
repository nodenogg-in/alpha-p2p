...
const useAppInternal = defineStore(MAIN_STORE_NAME, () => {
  ...
  const registerMicrocosm = (microcosm: Microcosm) => {
    if (microcosm.uri !== activeMicrocosm.value?.uri) {
  ...
  const updateIdentity = (data: IdentityAction['data'], peerId?: string) => {
    ...
    if (peerId) {
      peerIdentities.set(action.data.uid, {
        ...action.data,
        peerId
      })
    } else {
...
export const useMicrocosm = (namespace_id: string, microcosm_id: string) => {
  ...
  internal.registerMicrocosm(microcosmEntry)
  ...
  return defineStore(storeName, () => {
    ...
    internal.sync.onStateChange((s) => {
      connected.value = s
    })

    internal.sync.onPeersChange((newPeers) => {
    ...
    const createNode = (
      ...
      const action = makeCreateAction(
        newNodes.map((d) => ({
          ...
          id: d.id || createUuid(),
          identity: d.identity || internal.identity
        }))
      )
      ...
      if (remote) {
        ...
        internal.sync.sendAction(action)
      }
    }
    ...
    internal.sync.onAction(([syncURI, action, peerId]) => {
      if (uri === syncURI) {
        if (action.type === CREATE_ACTION_NAME) {
          ...
          internal.updateIdentity(action.data, peerId)
        }
      }
    })
