...
const generateKeyPair = async (): Promise<CryptoKeyPair> =>
  window.crypto.subtle.generateKey(algorithm, EXTRACTABLE, KEY_USAGES)
...
const exportKey = async (key: CryptoKey) => window.crypto.subtle.exportKey(KEY_EXPORT_TYPE, key)

const importKey = async (keyData: JsonWebKey, key_ops: KeyUsage[]) =>
  window.crypto.subtle.importKey(KEY_EXPORT_TYPE, keyData, algorithm, EXTRACTABLE, key_ops)
...
export const signData = async (privateKey: CryptoKey, data: object | string): Promise<string> => {
  ...
  const signature = await window.crypto.subtle.sign(algorithm, privateKey, encoded)
...
export const verifySignature = async (
  ...
  return window.crypto.subtle.verify(algorithm, publicKey, signatureArray, encoded)
}
