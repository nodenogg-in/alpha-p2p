    ...
    <TooltipProvider>
        <TooltipRoot :delayDuration="30">
            <TooltipTrigger :class="{ active, 'icon-button': true }" @click="e => $emit('click', e)">
                <slot></slot>
                <span class="command mini" v-if="props.keyCommand && props.showCommand">{{ props.keyCommand }}</span>
            </TooltipTrigger>
            <TooltipPortal>
                <TooltipContent class="tooltip-content" :side-offset="5">
                    {{ props.tooltip }}<span class="command" v-if="props.keyCommand">{{ props.keyCommand }}</span>
                </TooltipContent>
            </TooltipPortal>
        </TooltipRoot>
    </TooltipProvider>
