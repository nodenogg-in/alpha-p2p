...
import { useCurrentSpatialView, type Size } from '@/views/spatial'
import { fitAspectRatio } from '@/views/spatial/utils/geometry'
...
const transformCss = (x: number, y: number, s: number = 1) =>
    `translate(${px(x)}, ${px(y)}) scale(${s})`
...
<template>
    <div class="minimap-container" :style="size.container">
        <div class="minimap-viewport" :style="size.viewport"></div>
